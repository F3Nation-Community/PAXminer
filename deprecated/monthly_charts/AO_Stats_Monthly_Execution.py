...
This script executes the monthly AOcharter chart maker for all F3 regions using PAXminer.
'''
...
try:
    with mydb1.cursor() as cursor:
        sql = "SELECT * FROM paxminer.regions where firstf_channel IS NOT NULL AND send_region_stats = 1"
        cursor.execute(sql)
